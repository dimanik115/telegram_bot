...
async def make_changes_command(message: types.Message):
    ...
    await bot.send_message(message.from_user.id, 'Что хозяин ннадо???', reply_markup=admin_kb.button_case_admin)
...
async def load_desciption(message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        async with state.proxy() as data:
...
def register_handlers_admin(dp: Dispatcher):
    ...
    dp.register_message_handler(load_desciption, state=FSMAdmin.description)
